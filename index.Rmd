...
## Datasets
Two datasets were evaluated, **TMA 764** including `r nrow(DF_674)` patients and `r nrow(marker_674)` spots and **TMA 762** including `r nrow(DF_672)` patients and `r nrow(marker_672)` spots. The analysis describes the [clinicopathologic, treatment and outcome](descriptive_cpo.html) features corresponding to all patients included in the dataset.

## Descriptive analysis
The analysis also descibres the marker features as evaluated in the TMA spots, including the extent (i.e., percentage of positive cells), intensity (absent, weak, moderate and strong intensity), and H-score. The H-score was estimated by multipling the extent (0--100%) by the intensity (0 for absent, 1 for weak, 2 for moderate and 3 for strong intensity), ranging from 0 to 300.
...
For outcome analyses we included only the invasive cases (i.e., cases in which the TMA spot evaluated showed tumor cells). The outcome analysis included [regression modeling](outcome_regression.html) and [survival analysis](outcome_survival.html). For each feature (extent, intensity and H-score), each case was categorized as showing high or low expression using the median values of the feature grouped in 2 levels, considering either the absence (i.e., zero value) or the median value of the feature distribution.

Outcome variables included distant metastasis, tumor progression, overall mortality, and cancer-related mortality.
